...
class ProgressReport(object):
    def __init__(self, args, file, reportInterval, simulation, total, first=0):
        ...
        if first == 0:
            self._first = simulation.currentStep #DZ fixed to prevent pct < 0 for restart simulations
        else:
           self._first=first 
...
class SimulationOptions(object):
	...
	#===  Then include overrides ===#
        ...
        self.set_active('tempincrease',0.0,float,"Initial simulation temperature increase for annealing.")
        self.set_active('quenchinterval',2,int,"Block interval spacing in which to quench simulation from a higher temperature.")
        ...
        if self.quenchinterval <= 1:
            raise Exception("You need to set a finite quench interval spacing greater than 1!")
