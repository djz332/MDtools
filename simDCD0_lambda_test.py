...
def main(paramfile='params.in', overrides={}, quiktest=False, deviceid=None, progressreport=True, soluteRes=[0],lambdaLJ=1.0,lambdaQ=1.0): #simtime=2.0, T=298.0, NPT=True, LJcut=10.0, tail=True, useLJPME=False, rigidH2O=True, device=0, quiktest=False):
    ...
    if os.path.exists(args.restart_filename) and args.read_restart:
        ...
        #Test -- shift the solute in 1A increments and recalculate energy
        """
        npts = 31
        energies = []
        for ii in range(31):
            ...
            xyzsolute = xyz[soluteIndices,:]
            
            #print(xyzsolute)
            #print(state.getPotentialEnergy())
            
            xyz[soluteIndices,:] = xyzsolute + [0.1,0.0,0.0] * u.nanometer
            print(xyz[soluteIndices,:])
            simulation.context.setPositions(xyz)
            state = simulation.context.getState( getEnergy=True,getPositions=True )
            print(state.getPotentialEnergy())
            energies.append(state.getPotentialEnergy().value_in_unit(u.kilojoule/u.mole))
        
        print(state.getPositions(asNumpy=True)[soluteIndices,:])
        print(energies)
        """
