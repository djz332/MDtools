...
def main(paramfile='params.in', overrides={}, quiktest=False, deviceid=None, progressreport=True, customff=""): #simtime=2.0, T=298.0, NPT=True, LJcut=10.0, tail=True, useLJPME=False, rigidH2O=True, device=0, quiktest=False):
    ...
    if customff:
        for i in range(len(customff)):
            logger.info("Using customff: [{}]".format(customff[i]))
            with open(customff[i],'r') as f:
                ffcode = f.read()
            exec(ffcode,globals(),locals()) #python 3, need to pass in globals to allow exec to modify them (i.e. the system object)
        #print(sys.path)
        #sys.path.insert(1,'.')
        #exec("import {}".format(".".join(customff[i].split(".")[:-1])))
    else:
    ...
    if os.path.exists(args.restart_filename) and args.read_restart:
        ...
        #app.pdbfile.PDBFile.writeHeader(simulation.topology,open('equilibrated.pdb','w'))
        app.PDBFile.writeFile(simulation.topology, positions, open('equilibrated.pdb', 'a')) 
        if args.report_interval > 0:
    ...
    Uext = simulation.context.getParameter('Uext')
    dUext = simulation.context.getParameter('dUext')
    ...
    for iblock in range(0,nblocks):
        ...
        Uext = Uext + dUext
        simulation.context.setParameter('Uext',Uext)
        logger.info('Ramping Uext to {} kJ/mol'.format(simulation.context.getParameter('Uext')))       
...
if __name__ == "__main__":
    ...
    parser.add_argument("--customff", nargs='*',default="", type=str, help="List of custom force field python scripts to run after generating system")
